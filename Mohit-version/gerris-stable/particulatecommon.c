...
static void gfs_particulate_write (GtsObject * o, FILE * fp)
{
 ...
 fprintf (fp, " %g %g %g %g %g %g", p->mass, p->volume*pow(L, FTT_DIMENSION), p->time, p->shape_factor,
...
static void droplet_conversion(FttCell * cell, DropletsPar *p)
{
  ...
  if (i>0)  {
    ...
    gdouble distance = (radius.x*radius.x + radius.y*radius.y + radius.z*radius.z)/(pow((0.75/M_PI)*(p->drops[i-1].volume),(2./3.)));
